...
export default function ExpensesManagement() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setToast({
        message: err.response?.data?.message || err.response?.data?.error || 'Failed to save expense',
        type: 'error',
      });
    } finally {
      setSubmitting(false);
    ...
    setConfirm({
      ...
      onConfirm: async () => {
        try {
          ...
          setConfirm({ open: false });
          setToast({ 
            message: err.response?.data?.message || err.response?.data?.error || 'Failed to delete recurring template', 
            type: 'error' 
          });
        } finally {
          setLoadingStates(prev => ({ ...prev, [key]: false }));
              ...
              {formData.is_recurring && (
                <>
                  ...
                  <div>
                    ...
                    {editingExpense?.recurring_expense_id && (
                      <p className="text-xs text-amber-400 mt-1 flex items-start gap-1">
                        ...
                        <span>Cannot change due day if any monthly expenses are already paid. Delete this recurring template and create a new one instead.</span>
                      </p>
                    )}
